...
export default function WorkShiftsScreen() {
  ...
  const renderStatusBadge = (status: "Active" | "Inactive" | "Upcoming") => {
    ...
    let textStyle = styles.badgeTextActive;
    ...
    return (
      <View style={[styles.statusBadge, badgeStyle]}>
        <Text style={[styles.statusBadgeText, textStyle]}>{status}</Text>
      </View>
    );
  };

  const renderShiftCard = ({ item }: { item: ShiftRecord }) => (
    <View style={styles.card}>
      ...
      <View style={styles.cardHeader}>
        <View style={styles.employeeInfo}>
          ...
          <View style={styles.employeeDetails}>
            <Text style={styles.employeeName}>{item.employeeName || "Unassigned"}</Text>
      ...
      <View style={styles.cardBody}>
        <View style={styles.infoRow}>
          <Feather name="layers" size={14} color="#64748b" style={styles.infoIcon} />
          <Text style={styles.infoLabel}>Shift:</Text>
          <Text style={styles.infoValueBold}>{item.shiftName || "—"}</Text>
        </View>

        <View style={styles.infoRow}>
          <Feather name="clock" size={14} color="#64748b" style={styles.infoIcon} />
          <Text style={styles.infoLabel}>Time:</Text>
          <Text style={styles.infoValue}>{item.timeRange || "—"}</Text>
        </View>
      </View>
    </View>
  );
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={{ flex: 1, marginLeft: 8 }}>
          <Text style={styles.headerTitle}>Work Shifts</Text>
          <Text style={styles.headerSub}>Manage & track shift allotments</Text>
        </View>
      </View>

      {/* --- METRICS CARDS (Fixed 2x2 Grid - No Horizontal Scroll) --- */}
      ...
      {loading && !refreshing ? (
        <View style={styles.loaderBox}>
          <ActivityIndicator size="large" color="#2563eb" />
          <Text style={styles.loadingText}>Loading shift records...</Text>
        </View>
      ) : (
        <FlatList
          data={filteredShifts}
          ...
          ListEmptyComponent={
            <View style={styles.emptyCard}>
              ...
              <Text style={styles.emptyText}>No shift records found.</Text>
            </View>
          }
        />
      )}
...
const styles = StyleSheet.create({
  ...
  /* Metrics 2x2 Grid (No horizontal scrolling) */
  ...
  statLabel: { fontSize: 9, fontWeight: "700", color: "#64748b", letterSpacing: 0.5 },
  ...
  searchInputText: { flex: 1, fontSize: 13, color: "#0f172a", paddingVertical: 0 },
  filterButton: {
    flexDirection: "row",
    alignItems: "center",
  ...
  filterButtonText: { fontSize: 12, fontWeight: "600", color: "#2563eb", maxWidth: 85 },

  /* List Container & Cards */
  listContainer: { paddingHorizontal: 16, paddingBottom: 24, gap: 10 },
  card: {
    backgroundColor: "#ffffff",
    borderRadius: 12,
    padding: 14,
  ...
  avatarText: { fontSize: 13, fontWeight: "700", color: "#334155" },
  employeeDetails: { flex: 1 },
  employeeName: { fontSize: 14, fontWeight: "700", color: "#0f172a" },
  departmentText: { fontSize: 12, color: "#64748b" },
  ...
  badgeUpcoming: { backgroundColor: "#fef9c3" },
  badgeTextUpcoming: { color: "#a16207" },
  ...
  infoLabel: { fontSize: 12, color: "#64748b", width: 55 },
  infoValue: { fontSize: 12, color: "#334155", flex: 1 },
  infoValueBold: { fontSize: 12, fontWeight: "600", color: "#0f172a", flex: 1 },

  /* Loader & Empty States */
  loaderBox: { padding: 32, alignItems: "center" },
  loadingText: { marginTop: 8, fontSize: 13, color: "#64748b" },
  emptyCard: { padding: 40, alignItems: "center" },
  emptyText: { marginTop: 8, fontSize: 14, color: "#94a3b8" },
  ...
  dropdownOptionSelected: { backgroundColor: "#f8fafc" },
  dropdownOptionText: { fontSize: 13, color: "#334155" },
  dropdownOptionTextActive: { fontWeight: "700", color: "#2563eb" },
});