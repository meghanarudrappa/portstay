...
function RootLayoutContent() {
  ...
  useEffect(() => {
    // Prevent running initialization until fonts are ready
    if (!loaded) return;
    ...
    const initialize = async () => {
      try {
        ...
        if (isMounted) {
          setIsReady(true);
          // Hide splash screen ONLY after state readiness is confirmed
          await SplashScreen.hideAsync();
        }
      }
    };
  ...
  // // Perform navigation redirect once session and app setup are complete
  // useEffect(() => {
  //   if (!isReady) return;
  ...
  // // Keep rendering null until ready; Native Splash Screen covers this period
  // if (!loaded || !isReady) {
  //   return null;
  // }
